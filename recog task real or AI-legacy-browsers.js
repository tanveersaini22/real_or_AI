/****************************** 
 * Recog Task Real Or Ai *
 ******************************/


// store info about the experiment session:
let expName = 'recog task real or AI';  // from the Builder filename that created this script
let expInfo = {
...
psychoJS.openWindow({
  fullscr: true,
  color: new util.Color((1.0000, 0.9451, 0.7255)),
  units: 'height',
  waitBlanking: true,
  backgroundImage: '',
  backgroundFit: 'none',
...
psychoJS.start({
  ...
  resources: [
    // resources:
    {'name': 'new prac conds.csv', 'path': 'new prac conds.csv'},
    {'name': 'github_images/prac_AI_bedroom.png', 'path': 'github_images/prac_AI_bedroom.png'},
    {'name': 'github_images/prac_AI_dog.jpg', 'path': 'github_images/prac_AI_dog.jpg'},
    {'name': 'github_images/prac_real_lakehouse.jpg', 'path': 'github_images/prac_real_lakehouse.jpg'},
    {'name': 'new main conds.csv', 'path': 'new main conds.csv'},
...
async function experimentInit() {
  ...
  text_2 = new visual.TextStim({
    win: psychoJS.window,
    name: 'text_2',
    text: 'You will be shown a series of images.\n\nFor each image:\n\nDecide whether the image is real or AI-generated.\n\n\n\nPress SPACE to continue.',
  ...
  real_image = new visual.ButtonStim({
    win: psychoJS.window,
    name: 'real_image',
    text: 'Real Image',
    font: 'Times New Roman',
  ...
  real_image.clock = new util.Clock();
  
  ai_image = new visual.ButtonStim({
    win: psychoJS.window,
    name: 'ai_image',
  ...
  fetch('https://pipe.jspsych.org/api/data', {
      ...
      body: JSON.stringify({
          experimentID: 'rKZox9OMISfw', // * UPDATE WITH YOUR DATAPIPE EXPERIMENT ID *
          filename: filename,
          data: data,
      }),
...
function overall_instructionsRoutineBegin(snapshot) {
  return async function () {
    ...
    overall_instructionsComponents.push(text_2);
    overall_instructionsComponents.push(key_resp_2);
...
var prac_instructionsMaxDurationReached;
var _key_resp_3_allKeys;
var prac_instructionsMaxDuration;
...
function prac_instructionsRoutineBegin(snapshot) {
  return async function () {
    ...
    // keep track of whether this Routine was forcibly ended
    routineForceEnded = false;
    prac_instructionsClock.reset();
    routineTimer.reset();
    prac_instructionsMaxDurationReached = false;
    // update component parameters for each repeat
    key_resp_3.keys = undefined;
    key_resp_3.rt = undefined;
    _key_resp_3_allKeys = [];
    psychoJS.experiment.addData('prac_instructions.started', globalClock.getTime());
    prac_instructionsMaxDuration = null
    // keep track of which components have finished
    prac_instructionsComponents = [];
    prac_instructionsComponents.push(text_3);
    prac_instructionsComponents.push(key_resp_3);
...
function main_trialsLoopBegin(main_trialsLoopScheduler, snapshot) {
  return async function() {
    ...
    main_trials.forEach(function() {
      ...
      main_trialsLoopScheduler.add(trialRoutineBegin(snapshot));
      main_trialsLoopScheduler.add(trialRoutineEachFrame());
      main_trialsLoopScheduler.add(trialRoutineEnd(snapshot));
...
function trialRoutineBegin(snapshot) {
  return async function () {
    ...
    // reset real_image to account for continued clicks & clear times on/off
    real_image.reset()
    // reset ai_image to account for continued clicks & clear times on/off
    ai_image.reset()
    ...
    // keep track of which components have finished
    trialComponents = [];
    trialComponents.push(imageStim);
    trialComponents.push(real_image);
    trialComponents.push(ai_image);
...
function trialRoutineEachFrame() {
  return async function () {
    ...
    // *real_image* updates
    if (t >= 0 && real_image.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      real_image.tStart = t;  // (not accounting for frame time here)
    ...
    // if real_image is active this frame...
    if (real_image.status === PsychoJS.Status.STARTED) {
    }
    
    if (real_image.status === PsychoJS.Status.STARTED) {
      // check whether real_image has been pressed
      if (real_image.isClicked) {
        if (!real_image.wasClicked) {
          // store time of first click
          real_image.timesOn.push(real_image.clock.getTime());
          // store time clicked until
          real_image.timesOff.push(real_image.clock.getTime());
        } else {
          // update time clicked until;
          real_image.timesOff[real_image.timesOff.length - 1] = real_image.clock.getTime();
        }
        if (!real_image.wasClicked) {
          // end routine when real_image is clicked
          continueRoutine = false;
          
        }
        // if real_image is still clicked next frame, it is not a new click
        real_image.wasClicked = true;
      } else {
        // if real_image is clicked next frame, it is a new click
        real_image.wasClicked = false;
      }
    } else {
      // keep clock at 0 if real_image hasn't started / has finished
      real_image.clock.reset();
      // if real_image is clicked next frame, it is a new click
      real_image.wasClicked = false;
    }
    
    // *ai_image* updates
    if (t >= 0 && ai_image.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      ai_image.tStart = t;  // (not accounting for frame time here)
      ai_image.frameNStart = frameN;  // exact frame index
      
      ai_image.setAutoDraw(true);
    }
    
    
    // if ai_image is active this frame...
    if (ai_image.status === PsychoJS.Status.STARTED) {
    }
    
    if (ai_image.status === PsychoJS.Status.STARTED) {
      // check whether ai_image has been pressed
      if (ai_image.isClicked) {
        if (!ai_image.wasClicked) {
          // store time of first click
          ai_image.timesOn.push(ai_image.clock.getTime());
          // store time clicked until
          ai_image.timesOff.push(ai_image.clock.getTime());
        } else {
          // update time clicked until;
          ai_image.timesOff[ai_image.timesOff.length - 1] = ai_image.clock.getTime();
        }
        if (!ai_image.wasClicked) {
          // end routine when ai_image is clicked
          continueRoutine = false;
          
        }
        // if ai_image is still clicked next frame, it is not a new click
        ai_image.wasClicked = true;
      } else {
        // if ai_image is clicked next frame, it is a new click
        ai_image.wasClicked = false;
      }
    } else {
      // keep clock at 0 if ai_image hasn't started / has finished
      ai_image.clock.reset();
      // if ai_image is clicked next frame, it is a new click
      ai_image.wasClicked = false;
    }
    // *mouse* updates
    ...
    if (mouse.status === PsychoJS.Status.STARTED) {
      ...
      if (!_mouseButtons.every( (e,i,) => (e == prevButtonState[i]) )) { // button state changed?
        ...
        if (_mouseButtons.reduce( (e, acc) => (e+acc) ) > 0) { // state changed to a new click
          ...
          gotValidClick = false;
          mouse.clickableObjects = eval([real_image, ai_image])
...
function trialRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('real_image.numClicks', real_image.numClicks);
    psychoJS.experiment.addData('real_image.timesOn', real_image.timesOn);
    psychoJS.experiment.addData('real_image.timesOff', real_image.timesOff);
    psychoJS.experiment.addData('ai_image.numClicks', ai_image.numClicks);
    psychoJS.experiment.addData('ai_image.timesOn', ai_image.timesOn);
    psychoJS.experiment.addData('ai_image.timesOff', ai_image.timesOff);
...
var main_instructionsMaxDurationReached;
var _key_resp_allKeys;
var main_instructionsMaxDuration;
var main_instructionsComponents;
function main_instructionsRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    ...
    // keep track of whether this Routine was forcibly ended
    routineForceEnded = false;
    main_instructionsClock.reset();
    routineTimer.reset();
    main_instructionsMaxDurationReached = false;
    // update component parameters for each repeat
    key_resp.keys = undefined;
    key_resp.rt = undefined;
    _key_resp_allKeys = [];
    psychoJS.experiment.addData('main_instructions.started', globalClock.getTime());
    main_instructionsMaxDuration = null
    // keep track of which components have finished
    main_instructionsComponents = [];
    main_instructionsComponents.push(text);
    main_instructionsComponents.push(key_resp);
    
    main_instructionsComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function main_instructionsRoutineEachFrame() {
  return async function () {
...
function main_instructionsRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('main_instructions.stopped', globalClock.getTime());
    // update the trial handler
    if (currentLoop instanceof MultiStairHandler) {
...
function Thank_YouRoutineBegin(snapshot) {
  return async function () {
    ...
    // keep track of which components have finished
    Thank_YouComponents = [];
    Thank_YouComponents.push(text_5);
